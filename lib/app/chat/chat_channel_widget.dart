...
var _logger =
    MyLogger(logTag: "channel_messages_list_bloc.dart", enabled: true);

class NetworkChannelWidget extends StatefulWidget {
  NetworkChannelWidget();

  @override
  _NetworkChannelWidgetState createState() => _NetworkChannelWidgetState();
}
...
class _NetworkChannelWidgetState extends State<NetworkChannelWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    _logger.d(() => "build for ${channelBloc.channel.name} "
        "messages ${chatListMessagesBloc.messagesLoaderBloc.messages.length}");
                ...
                child: Padding(
                  ...
                  child: Row(
                    children: <Widget>[
                            ...
                            if (searchState.searchTerm?.isNotEmpty == true) {
                              if (searchState.foundMessages.isEmpty) {
                                ...
                                labelText = AppLocalizations.of(context).tr(
                                    ...
                                    ".found",
                                    args: [
                                      searchState.selectedFoundMessagePosition
                                          .toString(),
                                      searchState
                      ...
                      StreamBuilder<bool>(
                          stream:
                              chatListMessagesBloc.searchPreviousEnabledStream,
                          initialData:
                              chatListMessagesBloc.searchPreviousEnabled,
                          builder: (context, snapshot) {
                            var enabled = snapshot.data;
...
//                            .getNetworkItemIconColor(isChannelActive)
                              ...
                              onPressed: enabled
                                  ? () {
                                      chatListMessagesBloc
                                          .goToPreviousFoundMessage();
                                    }
                                  : null,
